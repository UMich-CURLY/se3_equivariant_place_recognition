...
def visualization_matplotlib():
    ...
    for axis in range(3): # rotating in different axis (x, y, z)
        for angle in range(0, 365, 5): # different angle in step of 5 degrees
            ...
            rotated_png_savename = folder_path+"png_pointclouds/"+"rotated_"+axis_list[axis]+"_"+str(angle)+".png"
            ...
            ax.grid(False)
            # ax.scatter(pointcloud[:, 0], pointcloud[:, 1], pointcloud[:, 2], c='C0', marker=".")
        ...
        for trans in np.arange(-100, 105, 5): # different value for translation
            ...
            translated_png_savename = folder_path+"png_pointclouds/"+"translated_"+axis_list[axis]+"_"+str(trans/100)+".png"
...
def generate_descriptors(model_name):
    ...
    def load_model(EVAL_MODEL, opt):
        # build model
        if EVAL_MODEL == 'e2pn_netvlad':
            from SPConvNets.models.e2pn_netvlad import E2PNNetVLAD
            model = E2PNNetVLAD(opt)
        elif EVAL_MODEL == 'e2pn_gem':
    ...
    # pretrained weight
    opt_oxford.resume_path = 'pretrained_model/e2pn_gem_exact_trainall.ckpt'
...
def plot_feature(model_name):
    ...
    for axis in range(3): # rotating in different axis (x, y, z)
        ...
        plt.xlabel('rotatation angle around %s-axis' % (str(axis_list)))
        ...
        plt.xlabel('translation in %s-axis' % (str(axis_list)))
        plt.ylabel('descriptor similarity')
...
def generate_video():
    ...
    # with imageio.get_writer(folder_path+"pointcloud_transformation.gif", mode='I', duration=0.05) as writer:  
    #     for axis in range(3): # rotating in different axis (x, y, z)
    #         for angle in range(0, 365, 5): # different angle in step of 5 degrees
    #             rotated_png_filename = folder_path+"png_pointclouds/"+"rotated_"+axis_list[axis]+"_"+str(angle)+".png"
