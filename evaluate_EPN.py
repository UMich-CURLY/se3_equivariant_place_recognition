...
#     dataset_folder = '/home/yanbhliu/data/kitti'
...
#     print("Evaluating using pretrain model", cfg.RESUME_FILENAME)
...
#             with open(os.path.join(baseline_result_folder, 'EPN_results.txt'), "r") as baseline_result_file:
...
def evaluate():
    ...
    opt_oxford.result_folder = 'results/kitti_evaluation_EPN_pointnetvlad'

    folderExist = os.path.exists(cfg.RESULTS_FOLDER)
    if not folderExist:
        os.makedirs(cfg.RESULTS_FOLDER)
        print("The new directory is created!")

    # build model
    if cfg.EVAL_MODEL == 'atten_epn_netvlad_select':
        from SPConvNets.models.epn_gcn_netvlad import Atten_EPN_NetVLAD_select
        model = Atten_EPN_NetVLAD_select(opt_oxford)
        model = model.to(device)
    elif cfg.EVAL_MODEL == 'epn_netvlad':
        from SPConvNets.models.epn_netvlad import EPNNetVLAD
        model = EPNNetVLAD(opt_oxford)
        model = model.to(device)
    elif cfg.EVAL_MODEL == 'atten_epn_netvlad':
